...
(function() {
  ...
  Nex.Widgets.Slider = (function(_super) {
    ...
    Slider.prototype.goto = function(slide) {
      var next, _ref, _ref1, _ref2, _ref3;
      if (!this.slides) {
      ...
      if (this.slides.length === 1) {
        ...
        this.slides[this.current].initialize();
        if ((_ref = this.slides[this.current]) != null) {
          _ref.active();
        }
      ...
      if ((_ref1 = this.slides[this.prev]) != null) {
        _ref1.el.removeClass('prevslide');
      }
      if ((_ref2 = this.slides[this.next]) != null) {
        _ref2.el.removeClass('nextslide');
      }
      this.current = next;
      this.prev = this.getPrev(this.current);
      this.next = this.getNext(this.current);
      this.slides[this.prev].el.addClass('prevslide');
      this.slides[this.prev].initialize();
      this.slides[this.next].el.addClass('nextslide');
      this.slides[this.next].initialize();
      this.slides[this.current].initialize();
      if ((_ref3 = this.slides[this.current]) != null) {
        _ref3.active();
      }
    ...
    Slider.prototype.cleanUp = function() {
      ...
      for (i = _i = 0, _len = _ref.length; _i < _len; i = ++_i) {
        slide = _ref[i];
        if (i === this.prev || i === this.current || i === this.next) {
          continue;
        }
        _results.push(slide != null ? slide.clear() : void 0);
      }
      return _results;
    };
  ...
  Slide = (function(_super) {
    ...
    function Slide() {
      ...
      if (this.asset.assets && this.subslides) {
        this.bind('ready', (function(_this) {
          return function(result) {
            _this.asset = result;
            return _this.bind('initialize', _this.render);
          };
        })(this));
        this.getData(this.asset.path);
      } else {
        this.bind('initialize', this.render);
      }
    }
    ...
    Slide.prototype.initialize = function() {
      if (Nex.Utils.toType(this.asset) === 'array') {
        return this.trigger('initialize', this.asset);
      } else {
        return this.trigger('initialize', [this.asset]);
      }
    };
    ...
    Slide.prototype.render = function(result) {
      var asset, col, html, i, _i, _j, _len, _len1, _ref, _ref1;
      if (this.initated) {
        return;
      }
      for (_i = 0, _len = result.length; _i < _len; _i++) {
